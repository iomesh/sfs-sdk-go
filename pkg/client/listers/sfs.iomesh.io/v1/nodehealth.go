...
type NodeHealthLister interface {
	...
	List(selector labels.Selector) (ret []*v1.NodeHealth, err error)
...
// List lists all NodeHealths in the indexer.
func (s *nodeHealthLister) List(selector labels.Selector) (ret []*v1.NodeHealth, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.NodeHealth))
	})
	return ret, err
}
...
type NodeHealthNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.NodeHealth, error)
...
// List lists all NodeHealths in the indexer for a given namespace.
func (s nodeHealthNamespaceLister) List(selector labels.Selector) (ret []*v1.NodeHealth, err error) {
...
// Get retrieves the NodeHealth from the indexer for a given namespace and name.
func (s nodeHealthNamespaceLister) Get(name string) (*v1.NodeHealth, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("nodehealth"), name)
	}
	return obj.(*v1.NodeHealth), nil
}
